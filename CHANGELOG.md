...
- **Collection tab.** A new tab is your own professions' recipe book: every recipe they can learn, the ones you already know ticked and greyed, the rest showing the skill they need against the skill you have, where they are taught, the specialization they require and the faction that can get them. Each profession is a collapsible section headed with your progress -- "Blacksmithing (185/385)" -- and can be left out from the options. Recipes you can go and learn right now come first, then the ones out of reach, then the ones already in the book. One button cycles the list between the whole collection, only what you have still to learn, and only what you could learn today. Hovering a row shows the full answer -- every vendor or trainer, each with its own zone -- for what the columns have to cut short.
- **Craft value and profit on the recipe details.** The details panel answers what a craft costs, what it sells for and what is left in one block, with the three figures in a column so they can be read against each other, and the quantity, the unit price and where the prices came from underneath. Profit is green or red, and labelled an upper bound rather than shown as fact when some reagents have no price.
...
- **Where a recipe comes from.** The Collection tab says whether a recipe is taught by a trainer, sold by a vendor, dropped, given by a quest, found by discovery, only available during a world event, or is a world drop — naming the vendor or creature and the zone where you will find them. Raid and dungeon recipes name the instance they drop in. The faction restriction is marked on the place it belongs to rather than in a column of its own -- most recipes are available to both sides, and the ones that are not are usually one vendor out of two. Every recipe in the database now says where it comes from.
- **Recipes that are not in the game are left out of the Collection.** A handful of recipes exist in the game's own data but were never implemented, or were removed and never came back — there is no trainer to visit and no pattern to buy. Listing them would be a hole in your collection you can never fill.
- **Where to learn a recipe, on the recipe details.** The details panel now has a "Where to learn" block for every recipe in the browser, not only the ones you have still to learn: who teaches or sells it and in which zone, one line per place, the faction restriction when there is one, and the skill and specialization it asks for.
- **Tabs can be hidden.** A new Tabs section in the options switches individual tabs off. The Recipes tab is always available.
- **Sort and filter the Collection by its columns.** Left-click a column header to sort by it, right-click one marked `[F]` to narrow the list: by status, by whether the skill is within reach, by where the recipe is taught, or by the specialization it needs. Clicking a sorted header a third time hands the list back to its natural order -- what you can learn now, in the order a profession is levelled. A "Clear filters" button appears whenever something is being held back.
- **The expansion and profit filters are in the window.** Which expansions to list, and whether to show only crafts worth more than their materials, used to be two trips into the options panel. Both are now buttons -- in the recipe browser's sidebar and on the Collection strip -- and they are the same settings, so changing one anywhere changes it everywhere. The expansion button says in its tooltip when a profession has its own setting that outranks it.
- **Recipe difficulty is coloured the way the game colours it.** The skill a recipe needs is now shown in the trade window's own four colours -- orange, yellow, green, grey -- against the recipe's own thresholds rather than an estimate. Two recipes that need the same skill can stop being worth crafting at very different points, and the column now says so.
- **Nearly every recipe knows the skill it takes.** The Skill column used to show a dash for a third of the collection, because the game's data does not state a requirement for a recipe you learn from a trainer. Ten recipes out of 2151 still have no answer; the rest have a number.
- **Where to learn now says where to walk.** Hovering a recipe in the Collection gives the map position of the vendor, trainer or creature it comes from, where one is known, and marks each place with the faction that can reach it -- so a recipe sold by an Alliance vendor in one city and a Horde vendor in another tells you which of the two is yours.
...
- **The options panel is organised in pages.** Browsing, Filters, Interface, Sync and Tools, one screenful at a time, instead of one long column that put the sync sliders below the bottom of the window on most resolutions.
- **The materials list is half as tall.** Each reagent used to spend a second line on the words "Unit" and "Total"; its cost now sits in the same money column the rest of the details panel uses, and the unit price appears only when more than one is needed.
- **Search is at the top-left of every list.** The Guild members and Collection tables kept theirs at the far right, on top of the filter button and on the opposite side of the window from the recipe browser's.
