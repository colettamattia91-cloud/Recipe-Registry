...
- **Missing recipes tab.** A new tab lists what your own professions can still learn: the recipe, the skill it needs against the skill you have, where it is taught, the specialization it requires and the faction that can get it. Recipes you can go and learn right now are listed first, grouped by profession, and each profession can be left out from the options. A **Ready to learn** switch narrows the list to the recipes whose skill and specialization you already meet, and hovering a row shows the full answer -- every vendor or trainer, each with its own zone -- for the rows the columns have to cut short.
...
- **Where a recipe comes from.** The Missing recipes tab now says whether a recipe is taught by a trainer, sold by a vendor, dropped, given by a quest, found by discovery, only available during a world event, or is a world drop — naming the vendor or creature and the zone where you will find them. Raid and dungeon recipes name the instance they drop in. A new Faction column marks the recipes only Alliance or only Horde can get; most are available to both, and those stay unmarked. Every recipe in the database now says where it comes from.
- **Recipes that are not in the game are no longer listed as missing.** A handful of recipes exist in the game's own data but were never implemented, or were removed and never came back — there is no trainer to visit and no pattern to buy. They used to appear in the Missing recipes tab as things you could go and learn.
- **Where to learn a recipe, on the recipe details.** The details panel now has a "Where to learn" block for every recipe in the browser, not only the missing ones: who teaches or sells it and in which zone, one line per place, the faction restriction when there is one, and the skill and specialization it asks for.
