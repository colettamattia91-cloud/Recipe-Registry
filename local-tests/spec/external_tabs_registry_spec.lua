...
Test.it("SetExternalTabLabel rewrites the registered label", function()
    local _, UI = freshUI()
    UI:RegisterExternalTab({ id = "orders", label = "Craft Orders" })
    local ok, err = UI:SetExternalTabLabel("orders", "Craft Orders (3)")
    Test.truthy(ok, err)
    Test.eq(UI:GetExternalTabSpec("orders").label, "Craft Orders (3)")
end)

Test.it("SetExternalTabLabel is a no-op when the label is unchanged", function()
    local _, UI = freshUI()
    UI:RegisterExternalTab({ id = "orders", label = "Craft Orders" })
    Test.truthy(UI:SetExternalTabLabel("orders", "Craft Orders"))
    Test.eq(UI:GetExternalTabSpec("orders").label, "Craft Orders")
end)

Test.it("SetExternalTabLabel rejects unknown tabs and invalid inputs", function()
    local _, UI = freshUI()
    local _, err1 = UI:SetExternalTabLabel("ghost", "Anything")
    Test.eq(err1, "unknown-tab")

    UI:RegisterExternalTab({ id = "orders", label = "Craft Orders" })
    local _, err2 = UI:SetExternalTabLabel("orders", "")
    Test.eq(err2, "missing-label")
    local _, err3 = UI:SetExternalTabLabel("", "X")
    Test.eq(err3, "missing-id")
end)
