...
Test.it("manages persistent debug log commands without requiring chat debug", function()
    local addon, wow = freshAddon()
    ...
    addon:Trace("manifest", "test manifest trace")
    addon:SlashHandler("debug log show 5 manifest")
    Test.truthy(printLogContains(wow, "Debug log enabled."), "debug log enable output")
    Test.truthy(printLogContains(wow, "Debug log entries: 1 scope=manifest"), "debug log show header")
    Test.truthy(printLogContains(wow, "scope=manifest test manifest trace"), "debug log entry output")
