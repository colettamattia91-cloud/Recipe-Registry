...
Test.it("prints the complete modern main command surface", function()
    ...
    Test.truthy(printLogContains(wow, "/rr version, /rr versions, /rr dump, /rr self [profession], /rr sync, /rr offline, /rr pull"), "diagnostic help")
...
Test.it("hides perf dump diagnostics unless debug is enabled", function()
    ...
    Test.truthy(printLogContains(wow, "Role=Client"), "sync dump status")
...
Test.it("hides sync and offline diagnostics unless debug is enabled", function()
    local addon, wow = freshAddon()

    addon:SlashHandler("sync")
    addon:SlashHandler("offline")
    Test.truthy(printLogDoesNotContain(wow, "Role=Client"), "sync output should be hidden without debug")
    Test.truthy(printLogDoesNotContain(wow, "Offline sync blocks served="), "offline telemetry should be hidden without debug")

    addon.debugMode = true
    addon:SlashHandler("sync")
    addon:SlashHandler("offline")
    Test.truthy(printLogContains(wow, "Role=Client"), "sync output")
    Test.truthy(printLogContains(wow, "Cache hit="), "sync cache output")
