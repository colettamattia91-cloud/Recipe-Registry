...
Test.it("auto tick does not seed requests from revision registry hints", function()
    ...
    data:GetOrCreateMember(peerKey).rev = 0

    addon.Sync:AutoSyncTick()

    Test.eq(Test.countKeys(addon.Sync.pendingRequests), 0, "auto tick should not queue revision-driven requests")
    Test.falsy(addon.Sync.inFlight, "auto tick should not create in-flight requests")
    Test.eq(countCommKind(wow, "REQ"), 0, "auto tick should not emit REQ traffic from revision hints")
end)
