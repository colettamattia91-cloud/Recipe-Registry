...
- **Collection tab.** A new tab is your own professions' recipe book: every recipe they can learn, the ones you already know ticked and greyed, the rest showing the skill they need against the skill you have, where they are taught, the specialization they require and the faction that can get them. Each profession is a collapsible section headed with your progress -- "Blacksmithing (185/385)" -- and can be left out from the options. Recipes you can go and learn right now come first, then the ones out of reach, then the ones already in the book. One control narrows the list from the whole collection to what you have still to learn, to what you could learn today. Hovering a row gives the full answer -- every vendor or trainer, each with its own zone and map position -- for what the columns have to cut short.
- **The Collection only lists what could actually be yours.** Recipes that exist in the game's data but were never implemented are left out, because there is no trainer to visit and no pattern to buy. So are the twenty-two engineering recipes taught only to certain classes -- a warrior's trainer will not teach the Deathblow X11 Goggles however high the warrior's engineering -- and hovering one you can learn says which classes it is taught to. A recipe behind a specialization you did not take stays in the full collection, since a specialization can be changed, but it is not counted among the holes you have still to fill.
- **Where a recipe comes from.** Every recipe in the database now says whether it is taught by a trainer, sold by a vendor, dropped, given by a quest, found by discovery, only available during a world event, or a world drop -- naming the vendor or creature and the zone, and the instance for a raid or dungeon drop. Where a trainer teaches it, the answer names the kind of trainer: "Master Engineering Trainer (Outland)", "Goblin Engineering Trainer (Kalimdor)". The faction restriction is marked on the place it belongs to rather than in a column of its own, because the recipes that have one are usually one vendor out of two. This is what the Collection's "Learned from" column reads, and it is also a "Where to learn" block on the recipe details, for every recipe in the browser rather than only the ones you are missing.
- **A Phase column on the Collection.** Some recipes only become obtainable when a later content phase opens -- the Black Temple ones in phase 3, the Sunwell ones in phase 5 -- so the column says which, for 129 of them. It is blank for everything available from the start, which is most of the book, and it sorts and filters like any other column: by a single phase, by "from the start", or by everything that arrived after launch. It catches a vendor who arrives with a later phase even where he stands in a city that was there on day one.
- **What a craft costs, what it sells for, and what is left.** The recipe details answer all three in one block, with the figures in a money column so they can be read against each other, and the quantity, the unit price and where the prices came from underneath. Profit is green or red, and labelled a best case rather than shown as fact when a reagent has no price. A "Show only profitable recipes" switch keeps the crafts worth more than their materials -- it needs TSM or Auctionator, and a recipe whose price cannot be worked out stays in the list, marked, rather than being hidden. The auction house's 5% cut can be subtracted from the profit figure; it is off by default, so "Sells for" stays gross and still works as the price to list at.
- **Vendor prices for materials.** Reagents sold by vendors -- vials, thread, spices -- often have no auctions at all, which used to leave them unpriced or priced far above what anyone pays. Every merchant window you open now teaches Recipe Registry what it charges, and a material costs whichever is cheaper between the vendor and the auction house. Only what a merchant always has in stock is recorded: a limited-supply item on a vendor is two of them and then a respawn timer, which is not a price you can plan a craft around.
- **The skill a recipe takes, in the game's own colours.** The Skill column used to show a dash for a third of the collection, because the game's data does not state a requirement for a recipe you learn from a trainer; ten recipes out of 2151 still have no answer and the rest have a number. That number is coloured the way the trade window colours it -- orange, yellow, green, grey -- against the recipe's own thresholds rather than an estimate, so two recipes needing the same skill can stop being worth crafting at very different points and the column says so.
- **The expansion and profit filters are in the window.** Which expansions to list, and whether to show only crafts worth more than their materials, used to be two trips into the options panel. Both are buttons now, and they are the same settings as the options panel, so changing one anywhere changes it everywhere. Expansions are chosen on the Collection strip -- the tab that lists a profession's whole book -- and its tooltip says when a profession has its own setting that outranks it.
- **Shift-click a recipe in the list to link it.** The crafted item goes wherever the game sends a shift-clicked item: a chat box, or the auction house search field. An enchant, which makes no item, links the enchant itself.
- **Tabs can be hidden.** A new Tabs section in the options switches individual tabs off. The Recipes tab is always available.

### Changed

- **The "Guild Addons" tab is now called "Guild members", and reads like the Collection.** It lists your guildmates with their addon status as one of the columns; the old name read as a list of addons. Both tables now sort, filter and clear the same way: left-click a column header to sort by it, right-click one to narrow the list, and click a sorted header a third time to hand the table back to its natural order. The choices are written out with the one in force ticked, and a narrowed column is marked in gold. The Guild members headers used to cycle a filter in place and write its value into the header -- "Version: Unknown ^ [F]" in a column too narrow to hold it -- so the only way to learn what the choices were was to keep clicking until they came round again. Each table has one control on its strip that says what is being held back and drops all of it at once.
- **The options panel is organised in pages.** Browsing, Filters, Interface, Economy, Sync and Tools, one screenful at a time, instead of one long column that put the sync sliders below the bottom of the window on most resolutions.
- **The materials list is a line per reagent.** Each one used to spend a second line on the words "Unit" and "Total"; its cost now sits in the money column the rest of the details panel uses, with the unit price beside the count when more than one is needed.
- **The Guild members search box is at the top-left**, where the recipe browser's is, instead of the far right on top of the filter button.

### Fixed

- **The window no longer sits on top of everything.** It was drawn in a layer above the bags and the game's own panels, so it stayed in front whatever you clicked. It now lives in the same layer as the character sheet and the spellbook, and whichever window you click last comes to the front.
